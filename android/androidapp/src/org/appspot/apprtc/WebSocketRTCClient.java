...
public class WebSocketRTCClient implements AppRTCClient, WebSocketChannelEvents {
  ...
  private static final String wssUrl= "ws://192.168.0.70:8443";
  private static final String stunUrl= "stun:40.71.44.212:3478";
  ...
  private void connectToRoomInternal() {
    ...
    Log.d(TAG, "Kumar2 - Connect to room: " + connectionUrl);
    roomState = ConnectionState.NEW;
    ...
    {
      String url = stunUrl;
      PeerConnection.IceServer stunServer = PeerConnection.IceServer.builder(url).createIceServer();
      iceServers.add(stunServer);
    }

    Log.d(TAG, "kumar - client ID: " + clientId);
    SignalingParameters params = new SignalingParameters(
            iceServers, false, clientId, connectionUrl, connectionUrl, null, null);
  ...
  private void signalingParametersReady(final SignalingParameters signalingParameters) {
    ...
    messageUrl = signalingParameters.wssUrl;
    leaveUrl = signalingParameters.wssUrl;
    Log.d(TAG, "Message URL: " + messageUrl);
    Log.d(TAG, "Leave URL: " + leaveUrl);
    Log.d(TAG, "roomId: " + connectionParameters.roomId);
    Log.d(TAG, "clientId: " + signalingParameters.clientId);
    ...
    //wsClient.hello(connectionParameters.roomId, signalingParameters.clientId);
  }
  ...
  public void sendAnswerSdp(final SessionDescription sdp) {
    handler.post(new Runnable() {
      ...
      public void run() {
        ...
        //jsonPut(json, "alias", connectionParameters.roomId);
        //jsonPut(json, "from", clientId);
        wsClient.sendAnswerSdp(json.toString());
      }
    });
  }
    ...
    handler.post(new Runnable() {
      ...
      public void run() {
        ...
        if (initiator) {
          ...
          wsClient.sendLocalIceCandidate(json.toString());
        }
      }
    });
  }
  ...
  public void onWebSocketMessage(final String msg) {
    ...
    try {
      ...
      if (json.has("msg")) {
      ...
      } else if (json.has("sdp")) {
        String sdpText = json.getString("sdp");
        json = new JSONObject(sdpText);
        String type = json.optString("type");
        ...
        String iceText = json.getString("ice");
        json = new JSONObject(iceText);
          String type = json.optString("type");
          if (type.equals("candidate")) {
            events.onRemoteIceCandidate(toJavaCandidate(json));
          }
